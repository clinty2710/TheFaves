        ...
        <div className="content-column">
          {Array.isArray(favorites) && favorites.map(fav => (
            <div key={fav._id} className="favorite-item">
              {fav.item_Type === 'movie' && fav.movie && fav.movie.poster_path ? (
                <img src={`https://image.tmdb.org/t/p/w500${fav.movie.poster_path}`} alt={fav.movie.title} />
              ) : fav.item_Type === 'music' && fav.music && fav.music.cover_image ? (
                <img src={fav.music.cover_image} alt={fav.music.title} />
              ) : fav.item_Type === 'book' && fav.book && fav.book.cover_image ? (
                <img src={fav.book.cover_image} alt={fav.book.title} />
              ) : (
                <div>No cover available</div>
              )}
              <p>{fav.item_Type === 'movie' ? fav.movie.title : fav.item_Type === 'music' ? fav.music.title : fav.book.title}</p>
              {fav.item_Type === 'book' && <p>{fav.book.author}</p>}
